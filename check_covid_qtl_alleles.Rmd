...
The direction of the arrow indicates whether the SNP is associated with increased (upwards arrow) or decreased (downwards arrow) risk of COVID. Note that this does indicate the direction of change in expression, but it meant to determine whether the SNP with top p value (or the top COVID SNP) has the same direction of effect. We are hoping to find cases where the p value of the top COVID SNP (blue) is similar to the p value of the top SNP overall (red), and in the same direction. In general it makes sense to ignore most QTL datasets where the top SNP has a p value worse than about 1e-3 or 1e-4, since these are just noise. I would only pay attention to datasets where there is a clear QTL effect.
